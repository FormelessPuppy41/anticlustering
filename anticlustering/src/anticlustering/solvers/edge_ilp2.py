...
class _ModelEdgeILPBase:
    ...
    def cached_pairs(self) -> List[Tuple[int, int]]:
        """
        Cached method to generate all unique pairs (i, j) with i < j.
        This is used to initialize the binary decision variables.
        """
        return [(i, j) for i in range(self.N) for j in range(i + 1, self.N)]
    
    @lru_cache(maxsize=None)
    def cached_triples(self) -> List[Tuple[int, int, int]]:
        """
        Cached method to generate all unique triplets (i, j, k) with i < j < k.
        This is used to enforce transitivity constraints.
        """
        return [
            (i, j, k)
    ...
    def _build_sets(self):
        ...
        self.m.PAIRS    = pyo.Set(
            dimen=2,
            initialize=self.cached_pairs(),
        )
        self.m.TRIPLES = pyo.Set(
            dimen=3, 
            initialize=self.cached_triples()
        )
    ...
    def solve(self) -> Tuple[
        ...
        tuple of (np.ndarray, float, Status | str, float | None)
            ...
            - runtime : float
                The time taken by the solver to find the solution, in seconds.
        ...
        return assignment, obj_val, status, gap, self.runtime_
